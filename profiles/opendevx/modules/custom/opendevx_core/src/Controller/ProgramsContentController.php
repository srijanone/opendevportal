...
namespace Drupal\opendevx_core\Controller;
    ...
    if ($group) {
      ...
      foreach ($bundle_names as $plugin_id => $bundle_name) {
        ...
        if ($plugin->getEntityBundle()) {
          $label = \Drupal::entityTypeManager()->getStorage('node_type')->load($plugin->getEntityBundle())->get('name');
          $build['#bundles'][$bundle_name] = [
            'label' => 'Create ' . $label,
            'description' => t('Adds %type content to programs both publicly and privately.', ['%type' => $label]),
